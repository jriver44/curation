    ...
    try:
        ...
        try:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                ("uuid-1", conn_id, "Thing", "thing", "Cat", "cat", 0, "2025-01-01T00:00:00Z", None),
            )
            conn.commit()
            assert False, "Expected CHECK constraint failure for quantity"
...
def test_unique_logical_item_per_collection(tmp_path: Path) -> None:
    ...
    try:
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("uuid-1", conn_id, "Padron X000", "padron x000", "Cigar", "cigar", 1, "2025-01-01T00:00:00Z", None),
        )
        ...
        try:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                ("uuid-2", conn_id, "padron x000", "padron x000", "cigar", "cigar", 1, "2025-01-01T00:00:00Z", None),
            )
        
            conn.commit()
            assert False, "Expected UNIQUE constraint failure for (collection_id, name_norm, category_norm)"
        except sqlite3.IntegrityError:
            pass
    finally:
        conn.close()